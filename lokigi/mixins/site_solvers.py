...
def _brute_force(
    ...
    for possible_solution in possible_combinations:
        if brute_force_keep_best_n is None and brute_force_keep_worst_n is None:
            # Keep all results
            single_solution_metrics = self.evaluate_single_solution_single_objective(
        ...
        # We store -score to simulate a Max-Heap using heapq
            ...
            if max_value_cutoff is None or (
                ...
                if brute_force_keep_best_n is not None:
                    if len(top_n_heap) < brute_force_keep_best_n and max_value <= max:
    ...
    if brute_force_keep_best_n is None and brute_force_keep_worst_n is None:
        ...
        if brute_force_keep_best_n is not None:
            best_list = [
                item[1] for item in sorted(top_n_heap, key=lambda x: x[0], reverse=True)
            ]
        if brute_force_keep_worst_n is not None:
            worst_list = [item[1] for item in sorted(bottom_n_heap, key=lambda x: x[0])]

        if brute_force_keep_best_n is not None and brute_force_keep_worst_n is None:
            return best_list
        elif brute_force_keep_worst_n is not None and brute_force_keep_best_n is None:
            return worst_list
        else:
...
def _greedy(
    ...
    for i in loop_iterations:
        ...
        best_indices = single_solution_metrics.show_solutions().head(1)["site_indices"][
            0
        ]
    ...
    while len(final_solutions_metrics) < num_solutions and attempts < max_attempts:
        ...
        if rng.random() > (1 - local_search_chance):
            ...
            while improved and swaps < max_swaps:
                ...
                current_metrics = _get_cached_metrics(tuple(sorted(current_solution)))
